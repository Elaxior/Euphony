...
export default function ApkTable({ entries, resolveDownloadUrl, formatBytes }) {
  return (
    <div className="glass-panel modern-surface overflow-hidden">
        ...
        {entries.map((entry, index) => (
          ...
          >
            ...
            <a
              ...
              className="btn-shimmer mt-4 inline-flex w-full items-center justify-center gap-2 rounded-lg border border-accent-300/30 bg-accent-500/15 px-3 py-2 text-xs font-semibold text-accent-100 transition hover:border-accent-200/50 hover:bg-accent-500/30"
            >
              <Download size={13} />
              Download APK
            </a>
      ...
      <div className="hidden overflow-x-auto md:block">
        <table className="w-full min-w-[640px] text-left text-sm">
          ...
          <tbody>
            {entries.map((entry) => (
              <tr key={entry.id} className="border-t border-white/10 text-slate-200 transition hover:bg-white/5">
                ...
                <td className="px-4 py-4">
                  <a
                    href={resolveDownloadUrl(entry.downloadUrl)}
                    download={entry.fileName || true}
                    className="btn-shimmer inline-flex items-center gap-2 rounded-lg border border-accent-300/30 bg-accent-500/15 px-3 py-2 text-xs font-semibold text-accent-100 transition hover:border-accent-200/50 hover:bg-accent-500/30"
                  >
                    <Download size={13} />
                    APK
                  </a>

...
import { Download, Loader2, MessageCircle, UploadCloud } from "lucide-react";
...
export default function App() {
  ...
  async function handleApkUpload(event) {
    event.preventDefault();
    if (!uploadForm.apk) {
      setStatus({ type: "error", text: "Please choose an APK file first." });
      return;
    }
    ...
    try {
      ...
      formData.append("adminKey", uploadForm.adminKey);
      ...
      const created = await response.json();
      setApks((current) => [created, ...current]);
      setLatestApk(created);
      setUploadForm({
        version: "",
        build: "stable",
        changelog: "",
      ...
      setStatus({ type: "success", text: "APK uploaded and published." });
    } catch {
      setStatus({
        type: "error",
        text: "Upload failed. Check admin key and backend availability."
      });
    } finally {
  ...
  return (
    <>
      ...
      <div className="relative overflow-hidden pb-20">
        ...
        <header className="section-shell grid gap-10 pt-28 md:grid-cols-[1.05fr_0.95fr] md:items-center">
          <motion.div
            initial={{ opacity: 0, y: 28 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
          >
            <p className="mb-4 inline-flex rounded-full border border-accent-400/40 bg-accent-500/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.25em] text-accent-300">
              Euphony Android App
            </p>
            <h1 className="font-heading text-4xl font-extrabold leading-tight text-white md:text-6xl">
              The <span className="gradient-text">premium music experience</span> for Android.
            </h1>
            ...
            <div className="mt-10 flex flex-wrap gap-4">
              <a
                ...
                className="inline-flex items-center gap-2 rounded-2xl bg-accent-500 px-6 py-3 font-bold text-white shadow-glow transition hover:translate-y-[-2px] hover:bg-accent-400"
              >
                <Download size={18} />
                Download APK
              </a>
              <a
                href="#download"
                className="inline-flex items-center rounded-2xl border border-white/20 px-6 py-3 font-semibold text-slate-200 transition hover:border-white/40 hover:bg-white/5"
              >
                View Changelog
              </a>
            </div>
          ...
          <Suspense
            fallback={
              <div className="glass-panel h-[360px] animate-pulse bg-gradient-to-br from-accent-500/20 to-cyan-500/10" />
            }
          >
            <Hero3D />
          </Suspense>
        ...
        <main className="mt-24 space-y-24">
          ...
          <section id="screenshots" className="section-shell">
            <SectionTitle
              ...
              subtitle="Swap these mock slides with your latest app screenshots anytime."
            />
            <div className="mt-10">
          ...
          <section id="reviews" className="section-shell">
            <SectionTitle
              ...
              subtitle="Collect testimonials and ratings directly from your audience."
            />

            <div className="mt-8 grid gap-8 lg:grid-cols-[1.3fr_0.7fr]">
              <div>
                <div className="mb-4 flex items-end gap-3">
                  <span className="font-heading text-5xl font-extrabold text-white">{averageRating}</span>
          ...
          <section id="download" className="section-shell">
            <SectionTitle
              ...
              subtitle="Publish new builds and keep users informed with clear release notes."
            />
            ...
            <div className="mt-10 grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
              ...
              <form className="glass-panel p-6" onSubmit={handleApkUpload}>
                <h3 className="font-heading text-xl font-bold">Upload APK</h3>
                <p className="mt-2 text-sm text-slate-400">
                  Admin panel for publishing new versions from this website.
                </p>
                <div className="mt-4 space-y-4">
                  <input
                    required
                  ...
                  />
                  <input
                    value={uploadForm.adminKey}
                    onChange={(event) =>
                      setUploadForm((current) => ({ ...current, adminKey: event.target.value }))
                    }
                    type="password"
                ...
                <button
                  ...
                  className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-500/80 px-4 py-3 font-semibold text-slate-950 transition hover:bg-cyan-400 disabled:cursor-not-allowed disabled:opacity-70"
                >
                  {isUploadingApk ? <Loader2 size={16} className="animate-spin" /> : null}
                  Publish Build
                </button>
              </form>
            </div>
          ...
          <section id="about" className="section-shell">
            ...
            />
            <div className="glass-panel mt-8 grid gap-5 p-8 md:grid-cols-3">
              <div>
                <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Developer</p>
                <p className="mt-1 text-lg font-semibold">Your Name / Team</p>
              </div>
              <div>
          ...
          <section id="contact" className="section-shell">
            <SectionTitle
              eyebrow="Contact"
              title="Collect feedback from listeners and testers"
              subtitle="Use this form to receive bug reports, feature requests, and partnership inquiries."
            />

            <form className="glass-panel mt-8 p-6 md:p-8" onSubmit={handleMessageSubmit}>
              <div className="grid gap-4 md:grid-cols-2">
                <input
                  required
      ...
      <AnimatePresence>
        {status.text ? (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 24 }}
            className={`fixed bottom-6 right-6 z-50 rounded-xl border px-4 py-3 text-sm backdrop-blur-xl ${
              status.type === "success"

...
class FakeQueueRepository(
    ...
    companion object {
        private const val AUTO_PLAY_MANUAL_GUARD_MS = 1200L
    }
    ...
    override suspend fun addToQueue(song: Song) {
        ...
        if (!_playbackMode.value.isShuffleEnabled) {
            _originalQueue.value = if (_originalQueue.value.isEmpty()) {
                updatedQueue
            } else {
    ...
    override suspend fun addNextToQueue(song: Song) {
        ...
        val insertAt = (currentIndex + 1).coerceAtMost(queue.size)
        ...
        if (!_playbackMode.value.isShuffleEnabled) {
            ...
            if (original.isEmpty()) {
                ...
                val currentSongVideoId = queue[currentIndex].videoId
                val originalCurrentIndex = original.indexOfFirst { it.videoId == currentSongVideoId }
                val originalInsertAt = if (originalCurrentIndex >= 0) {
                    (originalCurrentIndex + 1).coerceAtMost(original.size)
                } else {
                    original.size
                }

                _originalQueue.value = original.toMutableList().apply {
                    add(originalInsertAt, song)
                }.toList()
            }
        }
    }
    ...
    suspend fun playNext(isAutoPlay: Boolean = false) {
        ...
        if (isAutoPlay) {
            ...
            if (timeSinceManual < AUTO_PLAY_MANUAL_GUARD_MS) return
        } else {
            prepareManualInteraction()
        }
        ...
        when {
            ...
            !isFixedQueue && queue.isNotEmpty() && currentIndex in queue.indices -> {
                // Fallback: if queue reached end but we're in radio mode, fetch more and continue.
                addRelatedSongs(queue[currentIndex])

                val refreshedQueue = _queue.value
                if (currentIndex < refreshedQueue.size - 1) {
                    val nextIndex = currentIndex + 1
                    _currentIndex.value = nextIndex
                    playerRepository.playSong(refreshedQueue[nextIndex])
                }
            }
    ...
    private fun rankRelatedSongs(baseSong: Song, candidates: List<Song>): List<Song> {
        ...
        val diverseIds = diverse.map { it.videoId }.toSet()
        val fill = sortedCandidates.filterNot { it.videoId in diverseIds }
        return diverse + fill
    }

...
class PlaybackService : MediaSessionService() {
    ...
    companion object {
        ...
        private const val FOREGROUND_DEMOTION_DELAY_MS = 5000L
    }
    ...
    private var pendingForegroundDemotionJob: Job? = null
            ...
            if (player != null) {
                ...
                player.addListener(object : Player.Listener {
                    override fun onPlaybackStateChanged(playbackState: Int) {
                        ...
                        // Keep service alive briefly at end to allow seamless auto-next in background.
                        if ((playbackState == Player.STATE_ENDED || playbackState == Player.STATE_IDLE)
                            && !player.isPlaying) {
                            scheduleForegroundDemotion()
                        }
                    }
                    override fun onIsPlayingChanged(isPlaying: Boolean) {
                        updateNotification()
                        if (isPlaying) {
                            pendingForegroundDemotionJob?.cancel()
                            pendingForegroundDemotionJob = null
    ...
    private fun scheduleForegroundDemotion() {
        pendingForegroundDemotionJob?.cancel()
        pendingForegroundDemotionJob = serviceScope.launch {
            delay(FOREGROUND_DEMOTION_DELAY_MS)

            val stillNotPlaying = try {
                !AppContainer.providePlayerRepository().isPlaying.value
            } catch (_: Exception) {
                true
            }
